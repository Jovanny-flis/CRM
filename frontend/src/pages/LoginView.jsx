...
<div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 font-sans">
  <div className="bg-slate-800 max-w-md w-full rounded-3xl shadow-2xl border border-slate-700/50 overflow-hidden animate-in fade-in zoom-in duration-500">
    <div className="p-8">
      
      <div className="text-center mb-8">
        {/* Icono Minimalista Oscuro con Acento Naranja */}
        <div className="w-16 h-16 bg-slate-900/80 rounded-2xl mx-auto flex items-center justify-center mb-6 border border-slate-700/50 shadow-inner">
          <svg className="w-8 h-8 text-orange-500/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4"></path>
          </svg>
        </div>
        <h2 className="text-2xl font-extrabold text-white tracking-tight">Bienvenido al CRM</h2>
        <p className="text-slate-400 mt-2 text-sm font-medium">Ingresa tus credenciales para continuar</p>
      </div>

      {avisoSesion && (
        <div className="bg-amber-500/10 text-amber-500 p-4 rounded-xl text-sm mb-6 border border-amber-500/20 flex items-center gap-3 animate-in slide-in-from-top-2">
          <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
          <span className="font-medium">{avisoSesion}</span>
        </div>
      )}

      {/* Alerta de Error Oscura */}
      {error && (
        <div className="bg-red-500/10 text-red-400 p-4 rounded-xl text-sm mb-6 border border-red-500/20 flex items-center gap-3 animate-in slide-in-from-top-2">
          <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <span className="font-medium">{error}</span>
        </div>
      )}

      {/* Alerta de Éxito Oscura */}
      {mensajeExito && (
        <div className="bg-emerald-500/10 text-emerald-400 p-4 rounded-xl text-sm mb-6 border border-emerald-500/20 flex items-center gap-3 animate-in slide-in-from-top-2">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          ...
          <input 
            ...
            className="w-full bg-slate-900/50 border border-slate-700 text-white placeholder-slate-600 rounded-xl px-4 py-3 focus:bg-slate-900 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500/50 outline-none transition-all" 
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              ...
              className="text-xs font-bold text-slate-400 hover:text-orange-400 transition-colors disabled:opacity-50"
            >
        ...
        <button 
          type="submit" 
          disabled={cargando}
          className="w-full mt-8 px-4 py-3.5 bg-slate-700 text-white font-bold rounded-xl hover:bg-slate-600 border border-slate-600 hover:border-orange-500/30 shadow-lg shadow-slate-900 transition-all flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
