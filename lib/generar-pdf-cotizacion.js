...
const { esCotizacionEspecial } = require('./cotizacion-especial');
const {
  cotizacionAFormData,
...
const prepararDatosPdf = ({ formData, folio, nombreProspecto, modoEspecial = false }) => {
  ...
  const { res, errores } = calcularResultadosCotizacion(normalizado, { modoEspecial });
...
const generarPdfDesdeFormulario = async ({ formData, folio = null, nombreProspecto, modoEspecial = false }) => {
  const { docDefinition, nombreArchivo } = prepararDatosPdf({ formData, folio, nombreProspecto, modoEspecial });
...
const generarPdfDesdeCotizacion = async (cot, nombreProspecto) => {
  ...
  return generarPdfDesdeFormulario({
    ...
    modoEspecial: esCotizacionEspecial(cot),
  });
};
