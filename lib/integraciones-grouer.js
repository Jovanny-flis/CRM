...
const { cancelarClonSiExiste } = require('./envio-flising');
const { resolverMotivoCancelacion } = require('./envio-flising-reglas');
const {
    NOMBRE_EMPRESA,
    NOMBRE_PIPELINE,
    NOMBRE_ROBOT,
    ...
    async (req, res) => {
        ...
        try {
            ...
            const [filas] = await db.query(
                `SELECT o.lead_id, o.flising_lead_id, e.codigo AS estatus_codigo
            ...
            const body = req.body && typeof req.body === 'object' ? req.body : {};
            const motivo = resolverMotivoCancelacion(
                body.motivo || body.motivo_cancelacion,
            );

            if (esCancelado(origen)) {
                await cancelarClonSiExiste(pool, origen.lead_id, motivo);
                console.log(
            ...
            await db.query(
                ...
                [estatus.id, motivo, origen.lead_id, empresaId],
            );
            await cancelarClonSiExiste(pool, origen.lead_id, motivo);
    ...
    async (req, res) => {
        ...
        try {
            ...
            const [origenes] = await db.query(
                `SELECT o.solicitud_id, o.analisis_id, o.pdf_disponible, o.snapshot_json,
                        o.flising_lead_id, o.enviado_a_flising_at, o.asignado_flising_usuario_id,
                        uf.nombre AS agente_flising_nombre
                 FROM leads_origen_grouer o
                 LEFT JOIN usuarios uf ON uf.id = o.asignado_flising_usuario_id
                 WHERE o.lead_id = ?
                 LIMIT 1`,
                [id],
            );
            ...
            return res.json({
                ...
                flising_lead_id: origen.flising_lead_id || null,
                enviado_a_flising_at: origen.enviado_a_flising_at || null,
                asignado_flising_usuario_id: origen.asignado_flising_usuario_id || null,
                agente_flising_nombre: origen.agente_flising_nombre || null,
            });
        } catch (err) {
