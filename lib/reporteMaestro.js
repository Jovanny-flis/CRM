...
const ROLES_DIRECTORIO_MAESTRO = [
    'super_admin',
    'admin_empresa',
    'supervisor',
    'agente',
...
router.get(
    '/api/reportes/maestro-leads',
    verificarToken,
    revisarRol(ROLES_DIRECTORIO_MAESTRO),
    async (req, res) => {
        try {
            // El rol/usuario/empresa vienen del token verificado (req.usuarioCRM), nunca del query string.
            const { rol: role, id: usuario_id, empresa_id } = req.usuarioCRM;
            ...
            else if (role === 'admin_empresa' || role === 'riesgos') {
                // riesgos ve todas las cotizaciones/leads de su empresa, igual que admin_empresa, sin poder modificar nada.
