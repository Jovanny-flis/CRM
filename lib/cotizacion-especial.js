...
const aplicarRechazoLeadYCotizacion = async (pool, cotizacionId, leadId, empresaId) => {
    ...
    if (cot?.lote_id) {
        await db.query(
            'UPDATE cotizaciones SET autorizacion_estado = ? WHERE lote_id = ?',
            [ESTADO_AUTORIZACION.RECHAZADA, cot.lote_id],
        );
    } else {
        await db.query(
...
const assertPuedeVincularCotizacionEspecial = async (db, cotizacionId, leadIdDestino) => {
    ...
    const origenPermitido = cot.lead_id_origen_especial || cot.lead_id;
    if (origenPermitido && origenPermitido !== leadIdDestino) {
        const err = new Error(
            'Las cotizaciones especiales solo pueden vincularse al prospecto en el que se dieron de alta.',
        );
        err.status = 400;
        throw err;
    }
