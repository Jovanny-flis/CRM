...
const normalizarDatosCotizacion = (body) => {
    ...
    const d = {
        ...
        is_rentas_deposito: body.is_rentas_deposito ?? 0,
        rentas_deposito_cantidad: Number(body.rentas_deposito_cantidad) || 0,
        rentas_deposito_valor: Number(body.rentas_deposito_valor) || 0,
        es_especial: body.es_especial ? 1 : 0,
        autorizacion_estado: body.autorizacion_estado ?? null,
        lote_id: body.lote_id ?? null,
        lead_id_origen_especial: body.lead_id_origen_especial ?? null,
    };

    const esResidualPct = d.is_residual_pct === 1 || d.is_residual_pct === true;
    if (d.residual_valor != null && d.tasa_anual != null) {
        if (!esResidualPct && d.valor_activo > 0 && d.porcentaje_vr === 0) {
            d.porcentaje_vr = (d.residual_valor / d.valor_activo) * 100;
        }
        if (!esResidualPct && d.vr_calculado === 0) {
            d.vr_calculado = d.residual_valor;
        }
        if (esResidualPct && d.valor_activo > 0 && d.vr_calculado === 0) {
            d.vr_calculado = d.valor_activo * (d.residual_valor / 100);
        }
    }
...
const SQL_INSERT_COMPLETO = `
    ...
    (id, empresa_id, lead_id, usuario_id, tipo_arrendamiento, tipo_activo, marca, modelo, version, anio, nombre_activo,
     ...
     is_rentas_deposito, rentas_deposito_cantidad, rentas_deposito_valor,
     porcentaje_vr, vr_calculado, pago_inicial, renta_mensual_sin_iva, renta_mensual_con_iva,
     es_especial, autorizacion_estado, lote_id, lead_id_origen_especial) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`;
...
const paramsInsertCompleto = (id, d) => [
    ...
    d.is_rentas_deposito, d.rentas_deposito_cantidad, d.rentas_deposito_valor,
    d.porcentaje_vr, d.vr_calculado, d.pago_inicial,
    d.renta_mensual_sin_iva, d.renta_mensual_con_iva,
    d.es_especial ?? 0,
    d.autorizacion_estado ?? null,
    d.lote_id ?? null,
    d.lead_id_origen_especial ?? null,
];
